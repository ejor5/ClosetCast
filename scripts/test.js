...
function testLayoutEngine() {
  ...
  const gameStream = sandbox.window.closetCastLayout.buildLayout({ appMode: { mode: "normal" }, cameras, gameStreamActive: true });
  ...
  assert(gameStream.showStream && gameStream.stageClass.includes("has-game-stream"), "favorite games should render in the normal ambient media slot");
...
function testRendererCameraReconnects() {
  ...
  assert(stylesSource.includes(".mode-normal.has-game-stream") && stylesSource.includes("grid-area: ambient"), "favorite game streams should occupy the ambient media area");
  ...
  assert(rendererSource.includes("clickPlayInPage"), "stream automation should click a visible play control before fullscreen");
  assert(rendererSource.includes("collectCandidates(document)") && !rendererSource.includes("requestFullscreen"), "fullscreen tester should probe controls without fullscreening the host website");
